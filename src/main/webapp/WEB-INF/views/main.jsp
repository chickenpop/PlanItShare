...
<body>
	<main>
		...
		<section>
					...
					<table class="search" id="gosearch">
						<tr>
							...
							<td><input type="text" name="word" class="form-control"
								placeholder="도시명을 입력해주세요." required></td>
							<td>
			...
			<div id="planlist">
				<c:forEach items="${plist}" var="pdto">
					<div class="plan"
						...
						<div class="text">
							<div>${pdto.title}</div>
							<span><i class="fa-solid fa-user"></i>${pdto.author}</span> <span><i
								class="fa-solid fa-eye"></i>${pdto.readCount}</span> <span><i
								class="fa-solid fa-heart"></i>${pdto.likecnt}</span>
			...
			<div id="top">
				<a href="#side"><img src="/planitshare/asset/image/up.png"></a>
			</div>
	...
	<script>
		...
		$('.searchbtn').click(
				function(event) {
					...
					let time = Math.abs($(document).scrollTop()
							- $($(this).data('item')).position().top) * 0.5;

					$('html,body')
							.animate(
									{
										scrollTop : $($(this).data('item'))
												.position().top - 140
									}, time);
				});
							...
							$.ajax({
								...
								success : function(result) {
									...
									if (result.result == "1") {
										...
										$('#search-error')
												.append(
														'<td colspan="3" style="color:red">검색결과가 없습니다.</td>');
